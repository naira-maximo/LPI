...
public class MediaFormula {
    ...
    double primeiroConjunto = (P1 * 0.6 + ((E1 + E2) / 2) * 0.4); // 60% é o peso da primeira prova e a média das duas atividades tem peso de 40%
        return primeiroConjunto * 0.5 + (Math.max((primeiroConjunto - 5.9), 0) / (primeiroConjunto - 5.9)) * (API * 0.5) + X + (SUB * 0.2);
        // se for menor que 6, será um número negativo. Max vai retornar o menor valor entre os elementos
    }
